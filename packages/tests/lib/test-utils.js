...
describe('Test Contract Address Generation', function () {
    ...
    var getContractAddress = fathom_ethers_1.ethers.utils.getContractAddress;
    var Tests = [
...
describe('Test RLP Coder', function () {
    ...
    tests.forEach(function (test) {
        it(('RLP coder encoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.equal(fathom_ethers_1.ethers.utils.RLP.encode(test.decoded), test.encoded, 'RLP encoded - ' + test.name);
        });
    });
    tests.forEach(function (test) {
        it(('RLP coder decoded - ' + test.name), function () {
            this.timeout(120000);
            assert_1.default.ok(equals(fathom_ethers_1.ethers.utils.RLP.decode(test.encoded), test.decoded), 'RLP decoded - ' + test.name);
        });
    });
});
describe('Test Unit Conversion', function () {
    ...
    tests.forEach(function (test) {
        var wei = fathom_ethers_1.ethers.BigNumber.from(test.wei);
        it(('parses ' + test.ether + ' ether'), function () {
            assert_1.default.ok(fathom_ethers_1.ethers.utils.parseEther(test.ether.replace(/,/g, '')).eq(wei), 'parsing ether failed - ' + test.name);
        });
        it(('formats ' + wei.toString() + ' wei to ether'), function () {
            var actual = fathom_ethers_1.ethers.utils.formatEther(wei);
        ...
        ['kwei', 'mwei', 'gwei', 'szabo', 'finney', 'satoshi'].forEach(function (name) {
            ...
            if (test[name]) {
                it(('parses ' + test[name] + ' ' + name), function () {
                    this.timeout(120000);
                    assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits(test[name].replace(/,/g, ''), unitName).eq(wei), ('parsing ' + name + ' failed - ' + test.name));
                });
            }
            var expectedKey = (name + '_format');
            if (test[expectedKey]) {
                it(('formats ' + wei.toString() + ' wei to ' + name + ')'), function () {
                    var actual = fathom_ethers_1.ethers.utils.formatUnits(wei, unitName);
    ...
    it("formats with commify", function () {
        ...
        Object.keys(tests).forEach(function (test) {
            assert_1.default.equal(fathom_ethers_1.ethers.utils.commify(test), tests[test]);
        });
    });
    // See #2016; @TODO: Add more tests along these lines
    it("checks extra tests", function () {
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2", 0).eq(2), "folds trailing zeros without decimal: 2");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.", 0).eq(2), "folds trailing zeros without decimal: 2.");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.0", 0).eq(2), "folds trailing zeros without decimal: 2.0");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.00", 0).eq(2), "folds trailing zeros without decimal: 2.00");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2", 1).eq(20), "folds trailing zeros: 2");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.", 1).eq(20), "folds trailing zeros: 2.");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.0", 1).eq(20), "folds trailing zeros: 2.0");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.00", 1).eq(20), "folds trailing zeros: 2.00");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.5", 1).eq(25), "folds trailing zeros: 2.5");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.50", 1).eq(25), "folds trailing zeros: 2.50");
        assert_1.default.ok(fathom_ethers_1.ethers.utils.parseUnits("2.500", 1).eq(25), "folds trailing zeros: 2.500");
    });
});
describe('Test Namehash', function () {
    ...
    tests.forEach(function (test) {
        it(('computes namehash - "' + test.name + '"'), function () {
            this.timeout(120000);
            assert_1.default.equal(fathom_ethers_1.ethers.utils.namehash(test.name), test.expected, 'computes namehash(' + test.name + ')');
        });
    });
    var goodNames = [
        "ricmoo.eth",
    ...
    it("empty ENS name", function () {
        assert_1.default.ok(!fathom_ethers_1.ethers.utils.isValidName(""));
    });
    goodNames.forEach(function (name) {
        it("ENS namehash ok - " + name, function () {
            assert_1.default.ok(fathom_ethers_1.ethers.utils.isValidName(name));
            fathom_ethers_1.ethers.utils.namehash(name);
        });
    });
    badNames.forEach(function (name) {
        it("ENS namehash fails - " + name, function () {
            ...
            assert_1.default.throws(function () {
                var namehash = fathom_ethers_1.ethers.utils.namehash(name);
...
describe('Test ID Hash Functions', function () {
    ...
    tests.forEach(function (test) {
        it(('computes id - ' + test.name), function () {
            this.timeout(120000);
            var actual = fathom_ethers_1.ethers.utils.id(test.text);
...
describe('Test Solidity Hash Functions', function () {
    ...
    function test(funcName, testKey) {
        it("computes " + funcName + " correctly", function () {
            ...
            tests.forEach(function (test, index) {
                var actual = (fathom_ethers_1.ethers.utils)['solidity' + funcName](test.types, test.values);
    ...
    testsInvalid.forEach(function (type) {
        it("disallows invalid type \"" + type + "\"", function () {
            assert_1.default.throws(function () {
                fathom_ethers_1.ethers.utils.solidityPack([type], ["0x12"]);
            }, function (error) {
...
describe('Test Hash Functions', function () {
    ...
    it('computes keccak256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(fathom_ethers_1.ethers.utils.keccak256(test.data), test.keccak256, ('Keccak256 - ' + test.data));
        });
    });
    it('computes sha2-256 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(fathom_ethers_1.ethers.utils.sha256(test.data), test.sha256, ('SHA256 - ' + test.data));
        });
    });
    it('computes sha2-512 correctly', function () {
        this.timeout(120000);
        tests.forEach(function (test) {
            assert_1.default.equal(fathom_ethers_1.ethers.utils.sha512(test.data), test.sha512, ('SHA512 - ' + test.data));
        });
    });
});
describe('Test Solidity splitSignature', function () {
    it('splits a canonical signature', function () {
        ...
        for (var v = 27; v <= 28; v++) {
            var signature = fathom_ethers_1.ethers.utils.concat([r, s, [v]]);
            var sig = fathom_ethers_1.ethers.utils.splitSignature(signature);
    ...
    it('splits a legacy signature', function () {
        ...
        var s = '0xcafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7ecafe1a7e';
        for (var v = 27; v <= 28; v++) {
            var signature = fathom_ethers_1.ethers.utils.concat([r, s, [v - 27]]);
            var sig = fathom_ethers_1.ethers.utils.splitSignature(signature);
            assert_1.default.equal(sig.r, r, 'split r correctly');
            assert_1.default.equal(sig.s, s, 'split s correctly');
            assert_1.default.equal(sig.v, v, 'split v correctly');
...
describe('Test Base64 coder', function () {
    ...
    it('encodes and decodes the example from wikipedia', function () {
        ...
        var decoded = fathom_ethers_1.ethers.utils.toUtf8Bytes(decodedText);
        ...
        assert_1.default.equal(fathom_ethers_1.ethers.utils.base64.encode(decoded), encoded, 'encodes to base64 string');
        assert_1.default.equal(fathom_ethers_1.ethers.utils.toUtf8String(fathom_ethers_1.ethers.utils.base64.decode(encoded)), decodedText, 'decodes from base64 string');
    });
});
describe('Test UTF-8 coder', function () {
    var overlong = fathom_ethers_1.ethers.utils.Utf8ErrorReason.OVERLONG;
    var utf16Surrogate = fathom_ethers_1.ethers.utils.Utf8ErrorReason.UTF16_SURROGATE;
    var overrun = fathom_ethers_1.ethers.utils.Utf8ErrorReason.OVERRUN;
    var missingContinue = fathom_ethers_1.ethers.utils.Utf8ErrorReason.MISSING_CONTINUE;
    var unexpectedContinue = fathom_ethers_1.ethers.utils.Utf8ErrorReason.UNEXPECTED_CONTINUE;
    var outOfRange = fathom_ethers_1.ethers.utils.Utf8ErrorReason.OUT_OF_RANGE;
    var BadUTF = [
    ...
    BadUTF.forEach(function (test) {
        it('toUtf8String - ' + test.name, function () {
            ...
            var ignored = fathom_ethers_1.ethers.utils.toUtf8String(test.bytes, fathom_ethers_1.ethers.utils.Utf8ErrorFuncs.ignore);
            assert_1.default.equal(ignored, test.ignored, "ignoring errors matches");
            // Check the string using the replaceErrors conversion
            var replaced = fathom_ethers_1.ethers.utils.toUtf8String(test.bytes, fathom_ethers_1.ethers.utils.Utf8ErrorFuncs.replace);
            ...
            assert_1.default.throws(function () {
                var result = fathom_ethers_1.ethers.utils.toUtf8String(test.bytes);
    ...
    it('toUtf8String - random conversions', function () {
        ...
        for (var i = 0; i < 100000; i++) {
            ...
            var bytes = fathom_ethers_1.ethers.utils.toUtf8Bytes(str);
            var str2 = fathom_ethers_1.ethers.utils.toUtf8String(bytes);
            var escaped = JSON.parse(fathom_ethers_1.ethers.utils._toEscapedUtf8String(bytes));
...
describe('Test Bytes32String coder', function () {
    ...
    it("encodes an ens name", function () {
        ...
        var bytes32 = fathom_ethers_1.ethers.utils.formatBytes32String(str);
        var str2 = fathom_ethers_1.ethers.utils.parseBytes32String(bytes32);
...
function getHex(value) {
    return fathom_ethers_1.ethers.utils.hexlify(fathom_ethers_1.ethers.utils.toUtf8Bytes(value));
}
describe("Test nameprep", function () {
    ...
    Tests.forEach(function (test) {
        ...
        it(test.comment, function () {
            var input = fathom_ethers_1.ethers.utils.toUtf8String(test.input);
            if (test.output) {
                var expected = fathom_ethers_1.ethers.utils.toUtf8String(test.output);
                var actual = fathom_ethers_1.ethers.utils.nameprep(input);
...
describe("Test Signature Manipulation", function () {
    ...
    tests.forEach(function (test) {
        it("autofills partial signatures - " + test.name, function () {
            var address = fathom_ethers_1.ethers.utils.getAddress(test.accountAddress);
            var hash = fathom_ethers_1.ethers.utils.keccak256(test.unsignedTransaction);
            var data = fathom_ethers_1.ethers.utils.RLP.decode(test.signedTransaction);
            var s = data.pop(), r = data.pop(), v = parseInt(data.pop().substring(2), 16);
            var sig = fathom_ethers_1.ethers.utils.splitSignature({ r: r, s: s, v: v });
            {
                var addr = fathom_ethers_1.ethers.utils.recoverAddress(hash, {
                    r: r, s: s, v: v
                });
...
describe("Test Typed Transactions", function () {
    ...
    function equalsData(name, a, b, ifNull) {
        assert_1.default.equal(fathom_ethers_1.ethers.utils.hexlify(a), fathom_ethers_1.ethers.utils.hexlify((b == null) ? ifNull : b), name);
        return true;
    }
    function equalsNumber(name, a, b, ifNull) {
        assert_1.default.ok(fathom_ethers_1.ethers.BigNumber.from(a).eq((b == null) ? ifNull : b), name);
        return true;
    }
        ...
        it(test.name, function () {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            wallet = new fathom_ethers_1.ethers.Wallet(test.key);
                            return [4 /*yield*/, wallet.signTransaction(test.tx)];
                        case 1:
                            signed = _a.sent();
                            assert_1.default.equal(signed, test.signed, "signed transactions match");
                            assert_1.default.equal(fathom_ethers_1.ethers.utils.serializeTransaction(test.tx), test.unsigned, "unsigned transactions match");
                            {
                                tx = fathom_ethers_1.ethers.utils.parseTransaction(test.unsigned);
                                assert_1.default.ok(equalsTransaction("transaction", tx, test.tx), "all unsigned keys match");
                            }
                            {
                                tx = fathom_ethers_1.ethers.utils.parseTransaction(test.signed);
...
describe("BigNumber", function () {
    ...
    tests.forEach(function (test) {
        if (test.expectedValue == null) {
            it(test.testcase, function () {
                assert_1.default.throws(function () {
                    var value = fathom_ethers_1.ethers.BigNumber.from(test.value);
            ...
            it(test.testcase, function () {
                ...
                var value2 = fathom_ethers_1.ethers.BigNumber.from(value);
    ...
    [
        ...
        it("absolute value (" + test.value + ")", function () {
            var value = fathom_ethers_1.ethers.BigNumber.from(test.value);
            var expected = fathom_ethers_1.ethers.BigNumber.from(test.expected);
    ...
    it("Fails on junk with a length property", function () {
        ...
        assert_1.default.throws(function () {
            var value = fathom_ethers_1.ethers.BigNumber.from("100").add(junk);
            console.log("ERROR", value);
        }, function (error) {
            return true;
        });
...
describe("FixedNumber", function () {
    {
        ...
        Tests.forEach(function (test) {
            it("Create from=" + test.value, function () {
                var value = fathom_ethers_1.ethers.FixedNumber.from(test.value);
    ...
    }
    {
        ...
        Tests.forEach(function (test) {
            it("Rounding value=" + test.value + ", decimals=" + test.round, function () {
                var value = fathom_ethers_1.ethers.FixedNumber.from(test.value).round(test.round);
                assert_1.default.equal(value.toString(), test.expected);
            });
        });
    }
...
describe("Logger", function () {
    var logger = new fathom_ethers_1.ethers.utils.Logger("testing/0.0");
    it("setLogLevel", function () {
        fathom_ethers_1.ethers.utils.Logger.setLogLevel(fathom_ethers_1.ethers.utils.Logger.levels.DEBUG);
        fathom_ethers_1.ethers.utils.Logger.setLogLevel(fathom_ethers_1.ethers.utils.Logger.levels.INFO);
        fathom_ethers_1.ethers.utils.Logger.setLogLevel(fathom_ethers_1.ethers.utils.Logger.levels.WARNING);
        fathom_ethers_1.ethers.utils.Logger.setLogLevel(fathom_ethers_1.ethers.utils.Logger.levels.ERROR);
        fathom_ethers_1.ethers.utils.Logger.setLogLevel(fathom_ethers_1.ethers.utils.Logger.levels.OFF);
    ...
    it("checkArgumentCount - too few", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === fathom_ethers_1.ethers.utils.Logger.errors.MISSING_ARGUMENT;
        });
    });
    it("checkArgumentCount - too many", function () {
        assert_1.default.throws(function () {
            ...
            return error.code === fathom_ethers_1.ethers.utils.Logger.errors.UNEXPECTED_ARGUMENT;
        });
    });
});
describe("Base58 Coder", function () {
    it("decodes", function () {
        assert_1.default.equal(fathom_ethers_1.ethers.utils.toUtf8String(fathom_ethers_1.ethers.utils.base58.decode("JxF12TrwUP45BMd")), "Hello World");
    });
    it("encodes", function () {
        assert_1.default.equal(fathom_ethers_1.ethers.utils.base58.encode(fathom_ethers_1.ethers.utils.toUtf8Bytes("Hello World")), "JxF12TrwUP45BMd");
    });
});
/*
...
describe("EIP-712", function () {
    ...
    tests.forEach(function (test) {
        it("encoding " + test.name, function () {
            var encoder = fathom_ethers_1.ethers.utils._TypedDataEncoder.from(test.types);
            ...
            //console.log(test);
            assert_1.default.equal(fathom_ethers_1.ethers.utils._TypedDataEncoder.getPrimaryType(test.types), test.primaryType, "getPrimaryType");
            assert_1.default.equal(fathom_ethers_1.ethers.utils._TypedDataEncoder.hash(test.domain, test.types, test.data), test.digest, "digest");
        });
    });
    tests.forEach(function (test) {
        if (!test.privateKey) {
        ...
        it("signing " + test.name, function () {
            return __awaiter(this, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            wallet = new fathom_ethers_1.ethers.Wallet(test.privateKey);
...
describe("EIP-2930", function () {
    var Tests = [
        {
            ...
            tx: {
                ...
                chainId: 3,
                nonce: 13,
                gasPrice: fathom_ethers_1.ethers.BigNumber.from("0x65cf89a0"),
                gasLimit: fathom_ethers_1.ethers.BigNumber.from("0x5b68"),
                to: "0x32162F3581E88a5f62e8A61892B42C46E2c18f7b",
                value: fathom_ethers_1.ethers.BigNumber.from("0"),
                data: "0x",
                accessList: [
                    {
        ...
        },
        {
            ...
            tx: {
                ...
                nonce: 14,
                gasPrice: fathom_ethers_1.ethers.BigNumber.from("0x65cf89a0"),
                gasLimit: fathom_ethers_1.ethers.BigNumber.from("0x71ac"),
                to: "0x32162F3581E88a5f62e8A61892B42C46E2c18f7b",
                value: fathom_ethers_1.ethers.BigNumber.from("0"),
                data: "0x",
                accessList: [
    ...
    Tests.forEach(function (test) {
        it("tx:" + test.hash, function () {
            var tx = fathom_ethers_1.ethers.utils.parseTransaction(test.data);
            assert_1.default.equal(tx.hash, test.hash);
            var reason = deepEquals(tx, test.tx);
            assert_1.default.ok(reason == null, reason);
            var preimageData = fathom_ethers_1.ethers.utils.serializeTransaction((test.tx));
            assert_1.default.equal(preimageData, test.preimage);
            var data = fathom_ethers_1.ethers.utils.serializeTransaction((test.tx), test.tx);
