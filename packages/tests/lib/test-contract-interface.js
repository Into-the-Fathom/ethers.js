...
function equals(actual, expected) {
    ...
    // Uint8Array
    if (expected.buffer) {
        if (!fathom_ethers_1.ethers.utils.isHexString(actual)) {
            return false;
        }
        actual = fathom_ethers_1.ethers.utils.arrayify(actual);
    ...
    // Maybe address?
    try {
        var actualAddress = fathom_ethers_1.ethers.utils.getAddress(actual);
        var expectedAddress = fathom_ethers_1.ethers.utils.getAddress(expected);
...
function getValues(object, named) {
    ...
    switch (object.type) {
        ...
        case 'string':
            return object.value;
        case 'buffer':
            return fathom_ethers_1.ethers.utils.arrayify(object.value);
        case 'tuple':
...
describe('ABI Coder Encoding', function () {
    var coder = fathom_ethers_1.ethers.utils.defaultAbiCoder;
...
describe('Test Contract Events', function () {
    ...
    tests.forEach(function (test, index) {
        it(('decodes event parameters - ' + test.name + ' - ' + test.types), function () {
            this.timeout(120000);
            var iface = new fathom_ethers_1.ethers.utils.Interface(test.interface);
        ...
        it(('decodes event data - ' + test.name + ' - ' + test.types), function () {
            ...
            test.normalizedValues.forEach(function (expected, index) {
                if (test.indexed[index]) {
                    assert_1.default.ok((fathom_ethers_1.ethers.Contract.isIndexed(parsed[index]) && parsed[index].hash == null), 'parsed event data has empty Indexed - ' + index);
                }
                else {
...
describe('Test Interface Signatures', function () {
    ...
    tests.forEach(function (test) {
        it('derives the correct signature - ' + test.name, function () {
            var iface = new fathom_ethers_1.ethers.utils.Interface(test.abi);
            this.timeout(120000);
    ...
    it('derives correct description for human-readable ABI', function () {
        var iface = new fathom_ethers_1.ethers.utils.Interface(["function transfer(address from, uint amount)"]);
        [
            "transfer",
    ...
    it('parses transaction function', function () {
        ...
        var tx = fathom_ethers_1.ethers.utils.parseTransaction(rawTx);
...
describe('Test Number Coder', function () {
    ...
    it('encodes zero', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: fathom_ethers_1.ethers.constants.Zero }
        ];
    ...
    it('encodes one', function () {
        var ZeroValues = [
            ...
            { n: 'BigNumber', v: fathom_ethers_1.ethers.constants.One }
        ];
        var expected = oneHex;
        ['uint8', 'uint256', 'int8', 'int256'].forEach(function (type) {
            ZeroValues.forEach(function (value) {
    ...
    it('encodes negative one', function () {
        var Values = [
            ...
            { n: 'hex leading even length', v: '-0x0001' },
            { n: 'hex leading odd length', v: '-0x00001' },
            { n: 'BigNumber', v: fathom_ethers_1.ethers.constants.NegativeOne }
        ];
        var expected = maxHex;
    ...
    it('encodes full uint8 range', function () {
        for (var i = 0; i < 256; i++) {
            ...
            expected += fathom_ethers_1.ethers.utils.hexlify(i).substring(2);
    ...
    it('encodes full int8 range', function () {
        for (var i = -128; i < 128; i++) {
            ...
            if (i === -128) {
                ...
                expected += fathom_ethers_1.ethers.utils.hexlify(256 + i).substring(2);
            }
            else {
...
describe('Test Filters', function () {
    ...
    function doTest(test) {
        it(test.name, function () {
            var iface = new fathom_ethers_1.ethers.utils.Interface([test.signature]);
...
describe("Test ParamType Parser", function () {
    ...
    Tests.forEach(function (test) {
        it("allows correct modifiers " + JSON.stringify(test.type), function () {
            var paramType = fathom_ethers_1.ethers.utils.ParamType.from(test.type);
    ...
    it("testError1", function () {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        provider = new fathom_ethers_1.ethers.providers.InfuraProvider("ropsten", "49a0efa3aaee4fd99797bfa94d8ce2f1");
                        contract = new fathom_ethers_1.ethers.Contract(addr, [
                    ...
                    case 3:
                        error_1 = _a.sent();
                        assert_1.default.equal(error_1.code, fathom_ethers_1.ethers.utils.Logger.errors.CALL_EXCEPTION, "error.code");
...
describe("Additional test cases", function () {
    ...
    it("allows addresses without the 0x", function () {
        var iface = new fathom_ethers_1.ethers.utils.Interface([
