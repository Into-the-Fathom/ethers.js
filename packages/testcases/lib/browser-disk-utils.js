...
function loadTests(tag) {
    ...
    if (Cache[filename] == null) {
        ...
        try {
            ...
            var size = parseInt(comps[0]), compressedData = fathom_ethers_1.ethers.utils.base64.decode(comps[1]);
            var uncompressedData = new Uint8Array(size);
            (0, tiny_inflate_1.default)(compressedData, uncompressedData);
            Cache[filename] = JSON.parse(fathom_ethers_1.ethers.utils.toUtf8String(uncompressedData));
        }
        catch (error) {
...
function loadData(filename) {
    ...
    return fathom_ethers_1.ethers.utils.base64.decode(browser_data_json_1.default[filename]);
}
